...
-- --------------------------------------------------
-- Date Created: 10/22/2012 22:04:03
-- Generated from EDMX file: D:\MyPractices\DOTNET\EntityFramework\OrangeHRMSModel\OrangeHRMSModel\MyHRMSModel.edmx
-- --------------------------------------------------
...
CREATE TABLE [dbo].[HR_Employee] (
    ...
    [HR_LKP_JobTitle_JobID] int  NULL
);
GO
...
CREATE TABLE [dbo].[HR_Company] (
    ...
    [RegisteredEmailAddress] nvarchar(max)  NOT NULL,
    [CompanyDescription] nvarchar(max)  NOT NULL,
    [IsGovermentSector] nvarchar(max)  NOT NULL,
...
-- Creating primary key on [UserGuid], [UserName] in table 'HR_Emp_Users'
ALTER TABLE [dbo].[HR_Emp_Users]
ADD CONSTRAINT [PK_HR_Emp_Users]
    PRIMARY KEY CLUSTERED ([UserGuid], [UserName] ASC);
GO
...
-- Creating foreign key on [HR_LKP_JobTitle_JobID] in table 'HR_Employee'
ALTER TABLE [dbo].[HR_Employee]
ADD CONSTRAINT [FK_HR_EmployeeHR_LKP_JobTitle]
    FOREIGN KEY ([HR_LKP_JobTitle_JobID])
    REFERENCES [dbo].[HR_LKP_JobTitle]
        ([JobID])
    ON DELETE NO ACTION ON UPDATE NO ACTION;

-- Creating non-clustered index for FOREIGN KEY 'FK_HR_EmployeeHR_LKP_JobTitle'
CREATE INDEX [IX_FK_HR_EmployeeHR_LKP_JobTitle]
ON [dbo].[HR_Employee]
    ([HR_LKP_JobTitle_JobID]);
GO
